...
try:
    ...
    print("Missing required packages. Install with: pip install pyautogui numpy")
    sys.exit(1)
...
def human_move_to(x: int, y: int):
    ...
    # Verify position on macOS (moveTo can silently fail)
    for _ in range(3):
        time.sleep(0.05)
...
WINDOW_TITLE_KEYWORDS = ["quickbooks", "payment links", "get paid", "quickbooks.com"]
...
# Absolute screen coordinates — defaults, overridden by setup.md
_COORD_DEFAULTS = {
    "new_link": (0, 0),
    "multi_use": (0, 0),
    "next": (0, 0),
...
def load_waits(config_path: Path) -> dict:
    """Load button wait times from config. Format: - wait_button: `4.0`"""
    w = {}
    if not config_path.exists():
        return w
    for line in config_path.read_text(encoding="utf-8").splitlines():
...
def find_browser_window_titles() -> list:
    """List visible window titles via a shell-based approach (more reliable)."""
    try:
        ...
        for part in result.stdout.split(", "):
            part = part.strip()
            if part and any(kw in part.lower() for kw in WINDOW_TITLE_KEYWORDS):
                titles.append(part)
        return titles
    except Exception:
        return []
...
def get_browser_app_name():
    """Detect which browser is running and return its app name."""
    candidates = [
        "Google Chrome",
        "Safari",
        "Firefox",
    ...
    for name in candidates:
        try:
            result = subprocess.run(
                ["osascript", "-e",
                 f'tell application "System Events" to count (every process whose name is "{name}")'],
            ...
            continue
    # Fallback: check for any process containing "browser" or "sun" in name
    try:
        result = subprocess.run(
            ["osascript", "-e",
        ...
        for proc_name in result.stdout.split(", "):
            p = proc_name.strip().lower()
            if "sun" in p or "browser" in p:
...
def activate_browser_window(index: int):
    """Bring a matching browser to front. If auto-detect fails, warn but continue."""
    titles = find_browser_window_titles()
    if titles:
        if len(titles) < index:
            print(f"Warning: found {len(titles)} matching windows, requested index {index}. Using first match.")
            print(f"Available titles: {titles}")
        target_title = titles[min(index - 1, len(titles) - 1)]
        print(f"Matched window: {target_title}")
    else:
        print("Warning: could not detect matching window titles via AppleScript.")
        print("This is normal for some browsers (e.g. SunBrowser).")

    app_name = get_browser_app_name()
    if app_name:
        ...
        try:
            ...
            print("Warning: could not activate browser — ensure it is focused manually.")
    else:
        print("Warning: no running browser detected. Ensure the target browser window is focused.")
    time.sleep(1.5)
...
def log_click_event(name: str, event_type: str, details: str):
    ...
    try:
        ...
        log_file = DEBUG_DIR / f"click_log_{_time.strftime('%Y%m%d')}.txt"
...
def locate_button_image(name: str, search_region=None):
    ...
    if max_val < threshold:
        ...
        if max_val2 >= threshold:
            ...
            raise RuntimeError(f"Unable to locate image for '{name}' on the screen. max_val={max_val2:.3f}")
...
def capture_button_template(name: str, width: int = 200, height: int = 80, at_xy=None):
...
def get_click_position(name: str, force_coordinate: bool = False):
...
def capture_template_prompt(name: str, width: int = 200, height: int = 80, at_xy=None):
    if at_xy:
        print(f"截取 '{name}' 位置 ({at_xy[0]},{at_xy[1]})，尺寸 {width}x{height}")
    else:
        print(f"请将鼠标移动到 '{name}' 按钮上，然后按回车键截取 ({width}x{height})。")
        input("准备好后按 Enter... ")
    path = capture_button_template(name, width=width, height=height, at_xy=at_xy)
    print(f"已保存模板：{path}\n")
...
def click_at(name: str, force_coordinate: bool = False, sleep_after: float = 2.0):
    if name not in COORDINATES:
        raise KeyError(f"Unknown coordinate key: {name}")
    # force_coordinate only skips image recognition — all human simulation still applies
    abs_x, abs_y = get_click_position(name, force_coordinate=force_coordinate)
    ...
    mode = "coord" if force_coordinate else "image"
    print(f"Clicking {name} [{mode}] at screen ({abs_x}, {abs_y}){' [precision]' if precise else ''}")
    log_click_event(name, "CLICK", f"mode={mode} screen=({abs_x},{abs_y}) precise={precise}")
...
def create_payment_link(amount: str, note: str, dry_run: bool = False):
    ...
    for nav_attempt in range(max_nav_attempts):
        ...
        click_at("copy", sleep_after=WAITS.get("copy", 2.5))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create a QuickBooks payment link. (macOS)"
    )
    ...
    parser.add_argument(
        ...
        help="Browser window match index (default: 1)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--list-windows",
        action="store_true",
        help="List matching browser windows and exit.",
    )
    parser.add_argument(
        "--calibrate",
        action="store_true",
    ...
    parser.add_argument(
        "--capture-size",
        default="200x80",
        help="Template capture size WxH (default: 200x80)",
    )
    parser.add_argument(
        "--capture-all",
        action="store_true",
    ...
    parser.add_argument(
        "--capture-at",
        default=None,
        help="Capture at fixed X,Y instead of mouse (avoids hover effect)",
    )
    ...
    if args.list_windows:
        titles = find_browser_window_titles()
        if not titles:
            print(f"No visible windows containing any of {WINDOW_TITLE_KEYWORDS} found.")
            sys.exit(1)
    ...
    # Parse capture size: WxH
    _cap_w, _cap_h = 200, 80
    try:
        _cap_w, _cap_h = map(int, args.capture_size.split("x"))
    except Exception:
        pass

    # Parse capture-at: X,Y
    _cap_at = None
    if args.capture_at:
        try:
            _cap_at = tuple(map(int, args.capture_at.split(",")))
        except Exception:
            pass
    ...
    if args.capture_template:
        ...
        for name in args.capture_template:
            capture_template_prompt(name, width=_cap_w, height=_cap_h, at_xy=_cap_at)
        sys.exit(0)
    ...
    if args.capture_all:
        if args.image_dir:
            IMAGE_DIR = Path(args.image_dir)
        for name in IMAGE_FILE_NAMES:
            capture_template_prompt(name, width=_cap_w, height=_cap_h)
        sys.exit(0)
    ...
    try:
        activate_browser_window(args.window_index)
        if not args.dry_run:
            print("请立即切换到 SunBrowser 窗口...")
