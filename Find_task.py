...
def call_payment_script(script_path: Path, amount: str, note: str, window_index: int, image_mode: bool, dry_run: bool) -> None:
    ...
    if dry_run:
        cmd.append("--dry-run")
    print(f"调用 create_payment_link.py: {' '.join(cmd)}")
    # capture output so callers can inspect error messages and decide whether to retry
...
def scan_database(config: argparse.Namespace) -> None:
    ...
    if config.process:
        script_path = Path(__file__).resolve().parent / "create_payment_link.py"
        update_script_path = Path(__file__).resolve().parent / "update_link.py"
        check_login_path = Path(__file__).resolve().parent / "check_login.py"
        ...
        try:
            ...
            if row:
                ...
                if config.process:
                    ...
                    success = False
                    last_exc = None
                    # Try from configured window-index down to 1
                    for idx in range(int(config.window_index), 0, -1):
                        try:
                            ...
                            # If the error message doesn't look like a window-index mismatch, stop retrying
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="每分钟轮询 MySQL 数据库 status=0 的记录，可选择显示或处理模式。"
    )
    ...
    parser.add_argument("--window-index", type=int, default=3, help="adsPower 浏览器窗口索引，默认 3")
