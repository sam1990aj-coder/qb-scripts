...
def human_write(text: str):
    ...
    for char in text:
        # Gaussian delay around 55ms, sigma 35ms, clamped
        delay = max(0.015, min(0.18, random.gauss(0.055, 0.035)))
        pyautogui.write(char, _pause=False)
        time.sleep(delay)
        # Extra pause after spaces (word boundary)
        if char == " ":
            time.sleep(random.uniform(0.1, 0.35))
    # 10% chance: simulate typo — backspace last 1-3 chars and retype slower
...
# Per-button match threshold override (default 0.55 for Mac)
...
def get_browser_app_name():
    ...
    candidates = [
        ...
        "Brave Browser", "Opera", "Chromium", "SunBrowser",
    ]
...
def activate_browser_window(index: int):
    ...
    if app_name:
        ...
        print("Warning: no running browser detected.")
    time.sleep(1.5)
...
def navigate_to_payment_links() -> None:
    ...
    # # Step 1: Move to Create button, hover 2-4s (no click)
    ...
    # # Step 2: Drift right 45-50 px with human-like movement
    # drift = random.randint(45, 50)
    # print(f"导航: 向右移动 {drift} 像素...")
    # human_move_to(cx + drift, cy)

    # # Step 3: Move to Payment links button, hover ~1s, then click
    # print("导航: 移动到 Payment links 按钮...")
    # click_at("payment_links_nav", sleep_after=6.0)
    print("导航: 已跳过（假设已在 Payment links 页面）。")


def capture_button_template(name: str, width: int = 200, height: int = 80):
    IMAGE_DIR.mkdir(parents=True, exist_ok=True)
    # Reactivate browser (terminal had focus during input)
    app_name = get_browser_app_name()
    if app_name:
        subprocess.run(
            ["osascript", "-e", f'tell application "{app_name}" to activate'],
...
def check_and_login(password: str, window_index: int, dry_run: bool = False) -> bool:
    """Return True if login was performed, False if already logged in."""
    activate_browser_window(window_index)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="检测登录状态，如登出则自动登录并导航到 Payment links 页面。(macOS)")
    ...
    if args.list_windows:
        print("macOS: 请确认浏览器窗口标题包含以下关键词之一:")
        print(WINDOW_TITLE_KEYWORDS)
        sys.exit(0)
    ...
    if args.capture_template or args.capture_all:
